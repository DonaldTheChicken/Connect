...
class Game: #Handles a Game until someone wins or board fills
    ...
    def check_win(self):
        ...
        for row_num in range(self.row_count):
            for col_num in range(self.col_count - self.win_len + 1):
                current_colour = self.grid[row_num][col_num]
                if current_colour != 0:
                    if len(set(self.grid[row_num][:col_num+self.win_len])) == 1:
    ...
    def computer_move(self):
        ...
        #add a counter to the longest current line of its colour, thats valid
        pass
...
class MainWindow(QMainWindow):
    ...
    def create_board(self, board_rnum, board_cnum):
        ...
        for i in range(board_rnum):
            for j in range(board_cnum):
                ...
                self.board_buttons[i][j].setFixedSize(self.board_buttons[i][j].width(), self.board_buttons[i][j].width())
        ...
        for button_rnum in range(board_rnum):
            for button_cnum in range(board_cnum):
                ...
                self.board_buttons[button_rnum][button_cnum].clicked.connect(lambda checked, r=button_rnum, c=button_cnum: self.on_board_click(r, c))
    ...
    def on_board_click(self, clicked_rnum, clicked_cnum):
        #
        if not self.started:
            return 0

        self.game.insert(clicked_cnum)
        print(f"clicked row {clicked_rnum}, col {clicked_cnum}")

        self.display_counters()
        # self.game.print_board()
    
    def display_counters(self):
        #make the buttons that have couters coloured
        for i in range(self.game.row_count):
            for j in range(self.game.col_count):
                if self.game.grid[i][j] != 0:
                    # print(f"background-color: {self.game.colours[self.game.grid[i][j]]};")
                    self.board_buttons[i][j].setStyleSheet(f"background-color: {self.game.colours[self.game.grid[i][j]][1]};")
                    #border: none;border-radius: {self.board_buttons[0][0].width()//2}px;
